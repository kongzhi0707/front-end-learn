...
#### <div id="id7">七）实现一个渲染markdown文档loader</div>

需要安装依赖 md 转 html的依赖， 我这边使用 markdown-it
```
npm install markdown-it@12.0.6 -D 
```
在loaders目录下新建 beautify.js 和 md-loader.js 文件。

loaders/beautify.js 添加如下代码：
```
module.exports = function ModifyStructure(source) { 
  // 把 h3和h2开头的 换成数组
  const htmlList = source.replace(/<h3/g, '$*(<h3').replace(/<h2/g, '$*(<h2').split('$*(');

  // 给他们套上 div 且添加类名 card
...
如上代码：html = `module.exports = ${JSON.stringify(html)}` 解析的结果是一个html字符串，因此我们返回的代码是通过 module.exports 导出这个html字符串。因此在外界导入的时候就可以接收到这个HTML字符串。

然后我们需要在webpack.config.js 配置我们的 .md 文件的loader， webpack.config.js 所有配置代码如下：
...
在我们项目的根目录下 src/ 目录下新建 test.md， 内容如下：
